...
//
// The design under test is C++'s rather than Rust's. Soliton has no traits,
...
std::string instantiations_of(const std::string& contents) {
    ...
    std::string out;
    for (const soliton::typeck::Instantiation& instance : result.instantiations) {
        if (!out.empty()) {
            out += ' ';
        }
...
SOLITON_TEST(generics_instantiate_one_function_at_several_types) {
...
SOLITON_TEST(generics_record_one_instantiation_per_distinct_argument_list) {
    // Calling with the same types twice must reuse the instantiation,
    // or every call site would emit its own copy of the function.
    SOLITON_CHECK_EQ(instantiations_of(std::string{kMax} +
                                     "pub fn main() {\n"
                                     "    println(max(1, 2));\n"
                                     "    println(max(3, 4));\n"
                                     "    println(max(1.5, 2.5));\n"
...
SOLITON_TEST(generics_accept_a_parameter_with_no_operations_at_all) {
...
SOLITON_TEST(generics_infer_through_a_reference_parameter) {
...
SOLITON_TEST(generics_infer_through_an_array_element_type) {
    SOLITON_CHECK_EQ(instantiations_of("pub fn first<T>(v: &[T; 2]) -> T { return v[0]; }\n"
                                     "pub fn main() {\n"
                                     "    let a: [int; 2] = [1, 2];\n"
...
SOLITON_TEST(generics_support_several_type_parameters) {
    SOLITON_CHECK_EQ(instantiations_of("pub fn pick<A, B>(a: A, b: B) -> A { return a; }\n"
                                     "pub fn main() { println(pick(1, \"s\")); }\n"),
                   std::string{"pick<int, string>"});
}

SOLITON_TEST(generics_allow_a_generic_function_to_call_another) {
    accept("pub fn identity<T>(v: T) -> T { return v; }\n"
           "pub fn twice<T>(v: T) -> T { return identity(identity(v)); }\n"
           "pub fn main() { println(twice(1)); }\n");
}

SOLITON_TEST(generics_terminate_on_a_self_recursive_template) {
...
SOLITON_TEST(generics_instantiate_a_struct_at_several_types) {
...
SOLITON_TEST(generics_infer_a_struct_literals_arguments_from_its_fields) {
...
SOLITON_TEST(generics_treat_two_instantiations_as_different_types) {
    SOLITON_CHECK_EQ(first_error("struct Pair<T> { pub a: T, pub b: T, }\n"
                               "pub fn take(p: Pair<int>) { }\n"
                               "pub fn main() { take(Pair { a: 1.5, b: 2.5 }); }\n"),
                   std::string{"type mismatch"});
}

SOLITON_TEST(generics_support_a_struct_with_several_parameters) {
...
SOLITON_TEST(generics_infer_a_parameter_through_a_generic_struct) {
    SOLITON_CHECK_EQ(instantiations_of("struct Pair<T> { pub a: T, pub b: T, }\n"
...
SOLITON_TEST(generics_reject_the_wrong_number_of_type_arguments) {
    SOLITON_CHECK_EQ(first_error("struct Pair<T> { pub a: T, pub b: T, }\n"
                               "pub fn take(p: Pair<int, float>) { }\n"),
                   std::string{"`Pair` takes 1 type argument but 2 were given"});
}

SOLITON_TEST(generics_reject_type_arguments_on_a_plain_struct) {
...
SOLITON_TEST(generics_report_a_body_error_against_the_call_that_caused_it) {
    ...
    SOLITON_CHECK_EQ(errors.at(0).message, std::string{"cannot compare values of type `Point`"});
    SOLITON_CHECK_MSG(errors.at(0).notes.at(0).find("instantiated as `max<Point>`") !=
                        std::string::npos,
                    "note was: " + errors.at(0).notes.at(0));
}

SOLITON_TEST(generics_report_conflicting_inference_as_a_mismatch) {
    const std::vector<soliton::ast::Diagnostic> errors =
...
SOLITON_TEST(generics_reject_a_parameter_nothing_could_determine) {
...
SOLITON_TEST(generics_infer_a_constructor_from_what_it_is_bound_to) {
...
SOLITON_TEST(generics_say_where_a_constructors_type_has_to_come_from) {
    ...
    SOLITON_CHECK_MSG(says_annotate, "the error should say an annotation would settle it");
}

SOLITON_TEST(generics_still_prefer_the_arguments_over_the_binding) {
...
SOLITON_TEST(generics_reject_a_struct_literal_that_determines_nothing) {
    SOLITON_CHECK_EQ(first_error("struct Holder<T> { pub items: [T; 0], }\n"
                               "pub fn main() { let h = Holder { items: [] }; }\n"),
                   std::string{"cannot infer the element type of an empty array"});
}

SOLITON_TEST(generics_reject_the_wrong_argument_count_to_a_template) {
    SOLITON_CHECK_EQ(first_error("pub fn pair<T>(a: T, b: T) -> T { return a; }\n"
                               "pub fn main() { println(pair(1)); }\n"),
                   std::string{"this function takes 2 arguments but 1 was supplied"});
}

SOLITON_TEST(generics_reject_duplicate_type_parameters) {
    SOLITON_CHECK_EQ(first_error("pub fn f<T, T>(a: T, b: T) -> T { return a; }\n"),
                   std::string{"duplicate type parameter `T`"});
}

SOLITON_TEST(generics_reject_a_type_parameter_that_shadows_a_struct) {
    SOLITON_CHECK_EQ(first_error("struct T { pub x: int, }\n"
...
SOLITON_TEST(generic_impl_takes_its_type_from_the_receiver) {
...
SOLITON_TEST(generic_impl_serves_every_instantiation) {
    accept(std::string{kPair} +
           "pub fn main() {\n"
...
SOLITON_TEST(generic_impl_method_may_return_its_own_type) {
    accept(std::string{kPair} +
           "pub fn main() {\n"
           "    let p = Pair { left: 3, right: 7 };\n"
...
SOLITON_TEST(generic_impl_reports_a_return_of_the_wrong_type) {
    const std::vector<soliton::ast::Diagnostic> errors =
...
SOLITON_TEST(generic_method_infers_its_own_parameters_from_the_call) {
...
SOLITON_TEST(generic_method_may_add_parameters_to_a_generic_impl) {
...
SOLITON_TEST(generic_method_reports_a_parameter_it_cannot_infer) {
    SOLITON_CHECK_EQ(first_error("struct Counter { pub n: int, }\n"
                               "impl Counter { pub fn make<T>(self) -> T { return self.n; } }\n"),
                   std::string{"type parameter `T` cannot be inferred"});
}

SOLITON_TEST(generic_method_reports_one_parameter_asked_to_be_two_things) {
    const std::vector<soliton::ast::Diagnostic> errors =
...
SOLITON_TEST(generic_impl_refuses_a_parameter_shadowing_the_blocks) {
    SOLITON_CHECK_EQ(first_error("struct Pair<T> { pub left: T, }\n"
                               "impl<T> Pair<T> { pub fn f<T>(self) -> T { return self.left; } }\n"),
                   std::string{"type parameter `T` shadows the `impl` block's"});
}

SOLITON_TEST(generic_impl_requires_the_type_parameters_to_line_up) {
    SOLITON_CHECK_EQ(first_error("struct Pair<T, U> { pub left: T, pub right: U, }\n"
                               "impl<T> Pair<T> { pub fn f(self) -> T { return self.left; } }\n"),
                   std::string{"`Pair` takes 2 type parameters but this `impl` declares 1"});
}

SOLITON_TEST(generic_impl_on_a_type_that_is_not_generic_says_so) {
    SOLITON_CHECK_EQ(first_error("struct Point { pub x: int, }\n"
                               "impl<T> Point<T> { pub fn f(self) -> int { return self.x; } }\n"),
                   std::string{"`Point` is not generic"});
}

SOLITON_TEST(plain_impl_on_a_generic_type_says_what_to_write) {
    const std::vector<soliton::ast::Diagnostic> errors =
        reject("struct Pair<T> { pub left: T, }\n"
               "impl Pair { pub fn f(self) -> int { return 1; } }\n");
    SOLITON_CHECK_EQ(errors.at(0).message, std::string{"`Pair` is generic"});
    SOLITON_CHECK_MSG(errors.at(0).notes.at(0).find("impl<T> Pair<T>") != std::string::npos,
                    errors.at(0).notes.at(0));
}

SOLITON_TEST(generic_impl_methods_mangle_one_symbol_per_instantiation) {
    ...
    const std::string ir = compile_ir(std::string{kPair} +
                                      ...
                                      "    println(Pair { left: 1.5, right: 2.5 }.first());\n"
                                      "}\n");
    SOLITON_CHECK_MSG(ir.find("@Pair_first__int(") != std::string::npos, ir);
    SOLITON_CHECK_MSG(ir.find("@Pair_first__float(") != std::string::npos, ir);
}
...
SOLITON_TEST(generics_emit_one_function_per_instantiation) {
    if (!soliton::codegen::is_available()) {
        return;
    }
    ...
    SOLITON_CHECK_MSG(ir.find("@max__int") != std::string::npos, "no `max__int` in:\n" + ir);
    SOLITON_CHECK_MSG(ir.find("@max__float") != std::string::npos, "no `max__float` in:\n" + ir);
    // No `max` survives unmangled: a template is never emitted as itself.
    SOLITON_CHECK_MSG(ir.find("define i64 @max(") == std::string::npos,
                    "an uninstantiated template was emitted");
}

SOLITON_TEST(generics_emit_native_types_with_no_boxing) {
    if (!soliton::codegen::is_available()) {
        return;
    }
    const std::string ir = compile_ir(std::string{kMax} +
    ...
    SOLITON_CHECK_MSG(ir.find("define linkonce_odr i64 @max__int(i64") != std::string::npos,
                    "expected a native i64 signature in:\n" + ir);
}

SOLITON_TEST(generics_emit_a_separate_struct_layout_per_instantiation) {
    if (!soliton::codegen::is_available()) {
        return;
    }
    ...
    SOLITON_CHECK_MSG(ir.find("{ i64, i64 }") != std::string::npos,
                    "no int layout for Pair in:\n" + ir);
    SOLITON_CHECK_MSG(ir.find("{ double, double }") != std::string::npos,
                    "no float layout for Pair in:\n" + ir);
}

SOLITON_TEST(generics_leave_a_program_without_generics_unchanged) {
    ...
    SOLITON_CHECK_EQ(result.instantiations.size(), std::size_t{0});
}
