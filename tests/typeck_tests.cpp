...
EMBER_TEST(typeck_accepts_the_spec_example_program) {
...
EMBER_TEST(typeck_infers_let_types_from_the_initializer) {
...
EMBER_TEST(typeck_accepts_a_matching_let_annotation) {
    accept(in_main("let a: int = 1;\n    let b: float = 1.5;\n    println(a);\n    println(b);"));
}

EMBER_TEST(typeck_accepts_arithmetic_on_matching_numeric_types) {
...
EMBER_TEST(typeck_accepts_control_flow_with_bool_conditions) {
...
EMBER_TEST(typeck_accepts_arrays_and_indexing) {
    accept(in_main("let xs: [int; 3] = [1, 2, 3];\n"
                   "    println(xs[0]);\n"
                   "    println(len(xs));"));
}

EMBER_TEST(typeck_accepts_shadowing_in_a_nested_scope) {
...
EMBER_TEST(typeck_accepts_a_reference_parameter_given_a_value) {
...
EMBER_TEST(typeck_accepts_a_void_function_falling_off_the_end) {
    accept("pub fn nothing() { }\npub fn main() { nothing(); }\n");
}

EMBER_TEST(typeck_accepts_a_return_on_every_path_of_an_if_else) {
...
EMBER_TEST(typeck_rejects_a_let_annotation_that_does_not_match) {
...
EMBER_TEST(typeck_renders_the_spec_worked_example_verbatim) {
...
EMBER_TEST(typeck_rejects_a_non_bool_condition) {
    const std::vector<ember::ast::Diagnostic> errors = reject(in_main("if 1 { }"));
    EMBER_CHECK_EQ(errors.at(0).message, std::string{"type mismatch"});
    EMBER_CHECK_EQ(errors.at(0).label, std::string{"expected `bool`, found `int`"});
}

EMBER_TEST(typeck_rejects_a_returned_value_of_the_wrong_type) {
    const std::vector<ember::ast::Diagnostic> errors =
...
EMBER_TEST(typeck_rejects_assigning_the_wrong_type) {
    const std::vector<ember::ast::Diagnostic> errors =
        reject(in_main("let mut x = 1;\n    x = \"no\";"));
    EMBER_CHECK_EQ(errors.at(0).label, std::string{"expected `int`, found `string`"});
}

EMBER_TEST(typeck_rejects_a_non_int_index) {
    const std::vector<ember::ast::Diagnostic> errors =
...
EMBER_TEST(typeck_rejects_mixing_int_and_float) {
    const std::vector<ember::ast::Diagnostic> errors = reject(in_main("let x = 1 + 1.5;"));
    EMBER_CHECK_EQ(errors.at(0).message,
                   std::string{"cannot apply `+` to `int` and `float`"});
    EMBER_CHECK_EQ(errors.at(0).notes.at(0),
                   std::string{"`int` and `float` never mix implicitly in Ember (§4)"});
}
...
EMBER_TEST(typeck_accepts_casts_between_the_numeric_types) {
...
EMBER_TEST(typeck_lets_a_cast_bridge_int_and_float_arithmetic) {
...
EMBER_TEST(typeck_accepts_an_identity_cast) {
    accept(in_main("let a = 1 as int;\n    println(a);"));
}

EMBER_TEST(typeck_gives_a_cast_the_target_type) {
    const std::vector<ember::ast::Diagnostic> errors =
...
EMBER_TEST(typeck_rejects_casting_a_struct) {
    EMBER_CHECK_EQ(first_error(std::string{kPointProgram} +
                               "pub fn main() {\n"
                               "    let p = Point { x: 1, y: 2 };\n"
                               "    println(p as int);\n"
                               "}\n"),
...
EMBER_TEST(typeck_rejects_casting_a_string) {
    EMBER_CHECK_EQ(first_error(in_main("let x = \"12\" as int;")),
                   std::string{"cannot cast `string` to `int`"});
}

EMBER_TEST(typeck_rejects_a_float_annotation_on_an_int_literal) {
    const std::vector<ember::ast::Diagnostic> errors = reject(in_main("let x: float = 1;"));
    EMBER_CHECK_EQ(errors.at(0).label, std::string{"expected `float`, found `int`"});
}

EMBER_TEST(typeck_rejects_remainder_on_floats) {
    // C's `%` is integer-only; §9 says follow C on the low-level rules.
    EMBER_CHECK_EQ(first_error(in_main("let x = 1.5 % 2.0;")),
                   std::string{"cannot apply `%` to `float` and `float`"});
}

EMBER_TEST(typeck_rejects_arithmetic_on_non_numeric_types) {
    EMBER_CHECK_EQ(first_error(in_main("let x = \"a\" + \"b\";")),
                   std::string{"cannot apply `+` to `string` and `string`"});
    EMBER_CHECK_EQ(first_error(in_main("let x = true + false;")),
                   std::string{"cannot apply `+` to `bool` and `bool`"});
}

EMBER_TEST(typeck_rejects_logical_operators_on_non_bools) {
    EMBER_CHECK_EQ(first_error(in_main("let x = 1 && 2;")),
                   std::string{"cannot apply `&&` to `int` and `int`"});
}

EMBER_TEST(typeck_rejects_negating_a_bool_and_not_ing_a_number) {
    EMBER_CHECK_EQ(first_error(in_main("let x = -true;")),
                   std::string{"cannot apply `-` to `bool`"});
    EMBER_CHECK_EQ(first_error(in_main("let x = !1;")), std::string{"cannot apply `!` to `int`"});
}
...
EMBER_TEST(typeck_rejects_an_unknown_variable) {
    const std::vector<ember::ast::Diagnostic> errors = reject(in_main("println(nope);"));
    EMBER_CHECK_EQ(errors.at(0).message, std::string{"cannot find value `nope`"});
    EMBER_CHECK_EQ(errors.at(0).label, std::string{"not found in this scope"});
}

EMBER_TEST(typeck_rejects_an_unknown_function) {
    EMBER_CHECK_EQ(first_error(in_main("nope();")),
                   std::string{"cannot find function `nope`"});
}

EMBER_TEST(typeck_rejects_an_unknown_type) {
    EMBER_CHECK_EQ(first_error("pub fn f(p: Nope) { }\n"),
                   std::string{"cannot find type `Nope`"});
}

EMBER_TEST(typeck_suggests_a_close_variable_name) {
    ...
    EMBER_CHECK_EQ(errors.at(0).notes.at(0), std::string{"did you mean `count`?"});
}

EMBER_TEST(typeck_does_not_leak_a_binding_out_of_its_scope) {
    EMBER_CHECK_EQ(first_error(in_main("if true {\n"
                                       "        let inner = 1;\n"
                                       "    }\n"
                                       "    println(inner);")),
...
EMBER_TEST(typeck_rejects_too_few_arguments) {
    const std::vector<ember::ast::Diagnostic> errors =
    ...
    EMBER_CHECK_EQ(errors.at(0).label, std::string{"expected 2, found 1"});
}

EMBER_TEST(typeck_rejects_too_many_arguments) {
    EMBER_CHECK_EQ(first_error("pub fn one(a: int) { }\npub fn main() { one(1, 2); }\n"),
                   std::string{"this function takes 1 argument but 2 were supplied"});
}

EMBER_TEST(typeck_rejects_an_argument_of_the_wrong_type) {
    const std::vector<ember::ast::Diagnostic> errors =
...
EMBER_TEST(typeck_counts_method_arguments_without_the_receiver) {
...
EMBER_TEST(typeck_rejects_a_function_that_can_fall_off_the_end) {
    const std::vector<ember::ast::Diagnostic> errors = reject("pub fn f() -> int { }\n");
    EMBER_CHECK_EQ(errors.at(0).message, std::string{"missing return"});
    EMBER_CHECK_EQ(errors.at(0).label,
                   std::string{"this function must return `int` on every path"});
}

EMBER_TEST(typeck_rejects_an_if_without_an_else_as_the_only_return) {
    EMBER_CHECK_EQ(first_error("pub fn f(flag: bool) -> int {\n"
                               "    if flag {\n"
                               "        return 1;\n"
                               "    }\n"
                               "}\n"),
...
EMBER_TEST(typeck_does_not_count_a_while_loop_as_a_guaranteed_return) {
    // The condition may be false on the first test, so control can reach
    // the end of the body.
    EMBER_CHECK_EQ(first_error("pub fn f() -> int {\n"
                               "    while true {\n"
                               "        return 1;\n"
                               "    }\n"
                               "}\n"),
...
EMBER_TEST(typeck_rejects_a_bare_return_from_a_value_function) {
    EMBER_CHECK_EQ(first_error("pub fn f() -> int { return; }\n"),
                   std::string{"missing return value"});
}

EMBER_TEST(typeck_rejects_returning_a_value_from_a_void_function) {
    EMBER_CHECK_EQ(first_error("pub fn f() { return 1; }\n"),
...
EMBER_TEST(typeck_rejects_two_functions_with_the_same_name) {
    const std::vector<ember::ast::Diagnostic> errors =
        reject("pub fn f() { }\npub fn f() { }\n");
    EMBER_CHECK_EQ(errors.at(0).message, std::string{"duplicate definition of function `f`"});
    EMBER_CHECK_MSG(errors.at(0).notes.at(0).find("previously defined at") != std::string::npos,
                    "note was: " + errors.at(0).notes.at(0));
}

EMBER_TEST(typeck_rejects_two_structs_with_the_same_name) {
    EMBER_CHECK_EQ(first_error("struct S { x: int, }\nstruct S { y: int, }\n"),
                   std::string{"duplicate definition of type `S`"});
}

EMBER_TEST(typeck_rejects_duplicate_struct_fields) {
    EMBER_CHECK_EQ(first_error("struct S { x: int, x: int, }\n"),
                   std::string{"duplicate definition of field `x`"});
}

EMBER_TEST(typeck_rejects_two_methods_with_the_same_name) {
    EMBER_CHECK_EQ(first_error("struct S { x: int, }\n"
                               "impl S {\n"
                               "    fn f(&self) { }\n"
                               "    fn f(&self) { }\n"
...
EMBER_TEST(typeck_rejects_redeclaring_a_binding_in_the_same_scope) {
    EMBER_CHECK_EQ(first_error(in_main("let x = 1;\n    let x = 2;")),
                   std::string{"duplicate definition of `x`"});
}

EMBER_TEST(typeck_rejects_duplicate_parameters) {
    EMBER_CHECK_EQ(first_error("pub fn f(a: int, a: int) { }\n"),
                   std::string{"duplicate definition of parameter `a`"});
}

EMBER_TEST(typeck_rejects_redefining_an_intrinsic) {
    EMBER_CHECK_EQ(first_error("pub fn println(x: int) { }\n"),
...
EMBER_TEST(typeck_rejects_an_unknown_method) {
    const std::vector<ember::ast::Diagnostic> errors =
        reject(std::string{kPointProgram} +
               "pub fn main() {\n"
               "    let p = Point { x: 1, y: 2 };\n"
               "    println(p.area());\n"
               "}\n");
    EMBER_CHECK_EQ(errors.at(0).message, std::string{"no method `area` on type `Point`"});
    EMBER_CHECK_EQ(errors.at(0).label, std::string{"unknown method"});
}

EMBER_TEST(typeck_keeps_methods_out_of_the_global_function_namespace) {
    // §8 is explicit: `impl Point { fn f }` registers `f` on Point, not
    // as a free function.
    const std::vector<ember::ast::Diagnostic> errors =
        reject(std::string{kPointProgram} + "pub fn main() { distance_sq(); }\n");
    EMBER_CHECK_EQ(errors.at(0).message, std::string{"cannot find function `distance_sq`"});
    EMBER_CHECK_MSG(errors.at(0).notes.at(0).find("is a method on `Point`") != std::string::npos,
                    "note was: " + errors.at(0).notes.at(0));
}

EMBER_TEST(typeck_does_not_share_methods_between_structs) {
    EMBER_CHECK_EQ(first_error("struct A { x: int, }\n"
                               "struct B { x: int, }\n"
...
EMBER_TEST(typeck_distinguishes_a_field_from_a_method_in_diagnostics) {
    const std::vector<ember::ast::Diagnostic> errors =
        reject(std::string{kPointProgram} +
               "pub fn main() {\n"
               "    let p = Point { x: 1, y: 2 };\n"
               "    println(p.x());\n"
               "}\n");
    EMBER_CHECK_MSG(errors.at(0).notes.at(0).find("is a field, not a method") != std::string::npos,
                    "note was: " + errors.at(0).notes.at(0));
}

EMBER_TEST(typeck_resolves_a_method_through_a_reference_receiver) {
...
EMBER_TEST(typeck_gives_methods_the_mangled_name_codegen_will_emit) {
    ...
    EMBER_CHECK(method != result.methods.end());
    EMBER_CHECK_EQ(method->second.mangled_name, std::string{"Point_distance_sq"});
    EMBER_CHECK_EQ(method->second.param_types.size(), std::size_t{2});
    EMBER_CHECK_EQ(ember::typeck::to_string(method->second.param_types.at(0)),
                   std::string{"&Point"});
}
...
EMBER_TEST(typeck_rejects_an_unknown_field) {
    EMBER_CHECK_EQ(first_error(std::string{kPointProgram} +
                               "pub fn main() {\n"
                               "    let p = Point { x: 1, y: 2 };\n"
                               "    println(p.z);\n"
...
EMBER_TEST(typeck_rejects_a_struct_that_contains_itself_by_value) {
    EMBER_CHECK_EQ(first_error("struct Node { next: Node, }\n"),
                   std::string{"recursive type `Node` has infinite size"});
}

EMBER_TEST(typeck_rejects_mutually_recursive_structs) {
    ...
    EMBER_CHECK_EQ(errors.at(0).message, std::string{"recursive type `A` has infinite size"});
    EMBER_CHECK_EQ(errors.at(0).label, std::string{"`A` contains `B`, which contains `A`"});
}

EMBER_TEST(typeck_rejects_recursion_through_an_array_field) {
    ...
    EMBER_CHECK_EQ(errors.at(0).message, std::string{"recursive type `N` has infinite size"});
}

EMBER_TEST(typeck_rejects_a_longer_recursion_cycle) {
    EMBER_CHECK_EQ(first_error("struct A { pub b: B, }\n"
                               "struct B { pub c: C, }\n"
                               "struct C { pub a: A, }\n"),
                   std::string{"recursive type `A` has infinite size"});
}

EMBER_TEST(typeck_allows_recursion_broken_by_a_reference) {
...
EMBER_TEST(typeck_allows_a_struct_containing_a_different_struct) {
...
EMBER_TEST(typeck_allows_two_structs_sharing_a_field_type) {
...
EMBER_TEST(typeck_rejects_assigning_to_an_immutable_binding) {
    const std::vector<ember::ast::Diagnostic> errors =
        reject(in_main("let x = 1;\n    x = 2;"));
    EMBER_CHECK_EQ(errors.at(0).message,
                   std::string{"cannot assign to immutable binding `x`"});
    EMBER_CHECK_MSG(errors.at(0).notes.at(0).find("let mut x") != std::string::npos,
                    "note was: " + errors.at(0).notes.at(0));
}

EMBER_TEST(typeck_rejects_assigning_through_an_immutable_struct_binding) {
    EMBER_CHECK_EQ(first_error(std::string{kPointProgram} +
                               "pub fn main() {\n"
                               "    let p = Point { x: 1, y: 2 };\n"
                               "    p.x = 5;\n"
...
EMBER_TEST(typeck_accepts_assigning_through_a_mutable_struct_binding) {
    accept(std::string{kPointProgram} +
           "pub fn main() {\n"
...
EMBER_TEST(typeck_rejects_assigning_to_a_constant) {
    EMBER_CHECK_EQ(first_error("const LIMIT: int = 10;\npub fn main() { LIMIT = 20; }\n"),
                   std::string{"cannot assign to constant `LIMIT`"});
}

EMBER_TEST(typeck_rejects_assigning_to_a_parameter) {
    ...
    EMBER_CHECK_EQ(errors.at(0).message, std::string{"cannot assign to parameter `a`"});
    EMBER_CHECK_MSG(errors.at(0).notes.at(0).find("let mut` binding") != std::string::npos,
                    "note was: " + errors.at(0).notes.at(0));
}

EMBER_TEST(typeck_allows_writing_through_a_reference_parameter) {
...
EMBER_TEST(typeck_allows_writing_to_a_field_through_a_reference) {
...
EMBER_TEST(typeck_still_rejects_rebinding_a_reference_parameter) {
...
EMBER_TEST(typeck_rejects_an_invalid_assignment_target) {
    EMBER_CHECK_EQ(first_error(in_main("1 = 2;")), std::string{"invalid assignment target"});
}
...
EMBER_TEST(typeck_accepts_println_on_every_printable_type) {
...
EMBER_TEST(typeck_rejects_printing_a_struct) {
    EMBER_CHECK_EQ(first_error(std::string{kPointProgram} +
                               "pub fn main() {\n"
                               "    let p = Point { x: 1, y: 2 };\n"
                               "    println(p);\n"
...
EMBER_TEST(typeck_rejects_len_on_a_non_array) {
    EMBER_CHECK_EQ(first_error(in_main("println(len(1));")),
                   std::string{"cannot take the length of `int`"});
}

EMBER_TEST(typeck_rejects_println_with_the_wrong_argument_count) {
    EMBER_CHECK_EQ(first_error(in_main("println(1, 2);")),
                   std::string{"this function takes 1 argument but 2 were supplied"});
}
...
EMBER_TEST(typeck_reports_several_errors_in_one_run) {
    const std::vector<ember::ast::Diagnostic> errors =
...
EMBER_TEST(typeck_does_not_cascade_from_one_bad_expression) {
...
EMBER_TEST(typeck_records_a_type_for_every_expression_it_visits) {
    ...
    EMBER_CHECK(result.expr_types.size() > 10);

    for (const auto& [expr, type] : result.expr_types) {
        EMBER_CHECK_MSG(type != nullptr, "an expression was recorded with a null type");
    }
}
