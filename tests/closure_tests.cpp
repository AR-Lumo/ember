...
CINDER_TEST(closures_can_be_declared_and_called) {
    accept(in_main("let double = |x: int| -> int { return x * 2; };\n"
                   "    println(double(21));"));
}

CINDER_TEST(closures_take_no_parameters_when_written_with_empty_bars) {
...
CINDER_TEST(closures_may_return_nothing) {
    accept(in_main("let shout = |text: string| { println(text); };\n    shout(\"hi\");"));
}

CINDER_TEST(closures_still_allow_logical_or) {
    // The `||` token has two jobs now; this is the other one.
    accept(in_main("let a = true || false;\n    println(a);"));
}

CINDER_TEST(closures_can_be_called_more_than_once) {
...
CINDER_TEST(closures_may_take_no_arguments_in_their_type) {
...
CINDER_TEST(closures_capture_a_local_by_value) {
    accept(in_main("let offset = 100;\n"
                   "    let shift = |x: int| -> int { return x + offset; };\n"
                   "    println(shift(5));"));
}

CINDER_TEST(closures_record_only_what_the_body_mentions) {
    ...
    CINDER_CHECK_EQ(result.closure_captures.size(), std::size_t{1});
    CINDER_CHECK_EQ(result.closure_captures.begin()->second.size(), std::size_t{1});
}

CINDER_TEST(closures_do_not_capture_their_own_parameters) {
    const SourceFile source =
        make_source(in_main("let f = |x: int, y: int| -> int { return x + y; };\n"
                            "    println(f(1, 2));"));
    const CheckResult result = check(source);
    CINDER_CHECK(result.ok());
    CINDER_CHECK_EQ(result.closure_captures.begin()->second.size(), std::size_t{0});
}

CINDER_TEST(closures_do_not_capture_their_own_locals) {
...
CINDER_TEST(closures_capture_through_nesting) {
...
CINDER_TEST(closures_capture_an_owned_value_by_taking_it) {
...
CINDER_TEST(closures_move_an_owned_capture_out_of_the_enclosing_scope) {
    // Capturing by value means taking it. The closure owns it now, so
    // the scope that had it does not.
    CINDER_CHECK_EQ(first_error(in_main("let mut v: Vec<int> = new_vec();\n"
                                       "    let f = || -> int { return len(v); };\n"
                                       "    println(len(v));")),
                   std::string{"use of moved value `v`"});
}

CINDER_TEST(closures_do_not_move_a_capture_that_copies) {
...
CINDER_TEST(closures_with_owned_captures_carry_a_drop_function) {
    ...
    const std::string owned = compile_ir(in_main("let mut v: Vec<int> = new_vec();\n"
                                                 "    let f = || -> int { return len(v); };\n"
                                                 "    println(f());"));
    CINDER_CHECK_MSG(owned.find("cinder_closure_drop_") != std::string::npos,
    ...
    const std::string plain = compile_ir(in_main("let n = 5;\n"
                                                 "    let f = || -> int { return n; };\n"
                                                 "    println(f());"));
    CINDER_CHECK_MSG(plain.find("cinder_closure_drop_") == std::string::npos,
...
CINDER_TEST(closures_take_their_parameter_types_from_what_they_are_passed_to) {
...
CINDER_TEST(closures_take_their_return_type_the_same_way) {
...
CINDER_TEST(closures_still_accept_types_written_out) {
    accept(std::string{kMap} +
           in_main("let mut v: Vec<int> = new_vec();\n"
                   "    map_in_place(v, |x: int| -> int { return x * 2; });"));
}

CINDER_TEST(closures_check_an_inferred_body_against_the_expected_type) {
...
CINDER_TEST(closures_report_a_parameter_with_nothing_to_infer_from) {
    const std::vector<cinder::ast::Diagnostic> errors =
        reject(in_main("let f = |x| { println(x); };\n    f(1);"));
    CINDER_CHECK_EQ(errors.at(0).message, std::string{"cannot work out the type of `x`"});
    CINDER_CHECK_MSG(errors.at(0).notes.at(0).find("what it is passed to") != std::string::npos,
...
CINDER_TEST(closures_report_a_wrong_argument_type) {
    const std::vector<cinder::ast::Diagnostic> errors =
        reject(in_main("let f = |x: int| -> int { return x; };\n    println(f(true));"));
    CINDER_CHECK_EQ(errors.at(0).label, std::string{"expected `int`, found `bool`"});
}

CINDER_TEST(closures_report_a_wrong_argument_count) {
    CINDER_CHECK_EQ(first_error(in_main("let f = |x: int| -> int { return x; };\n"
                                       "    println(f(1, 2));")),
                   std::string{"this function takes 1 argument but 2 were supplied"});
}

CINDER_TEST(closures_report_a_missing_return) {
    CINDER_CHECK_EQ(first_error(in_main("let f = |x: int| -> int { println(x); };")),
                   std::string{"missing return"});
}

CINDER_TEST(closures_report_a_body_returning_the_wrong_type) {
    const std::vector<cinder::ast::Diagnostic> errors =
        reject(in_main("let f = |x: int| -> int { return true; };"));
    CINDER_CHECK_EQ(errors.at(0).label, std::string{"expected `int`, found `bool`"});
}

CINDER_TEST(closures_reject_calling_a_non_function_value) {
    CINDER_CHECK_EQ(first_error(in_main("let x = 1;\n    println(x(2));")),
                   std::string{"`x` is not callable"});
}

CINDER_TEST(closures_reject_a_type_mismatch_against_a_function_type) {
    const std::vector<cinder::ast::Diagnostic> errors =
...
CINDER_TEST(closures_move_when_passed_by_value) {
    CINDER_CHECK_EQ(first_error("pub fn consume(f: fn(int) -> int) { }\n" +
...
CINDER_TEST(closures_do_not_move_when_borrowed) {
...
CINDER_TEST(closures_move_on_assignment) {
    CINDER_CHECK_EQ(first_error(in_main("let f = |x: int| -> int { return x; };\n"
                                       "    let g = f;\n"
                                       "    println(f(1));")),
                   std::string{"use of moved value `f`"});
...
CINDER_TEST(closures_lift_to_real_functions) {
    if (!cinder::codegen::is_available()) {
        return;
    }
    const std::string ir = compile_ir(in_main("let f = |x: int| -> int { return x * 2; };\n"
                                              "    println(f(21));"));
    CINDER_CHECK_MSG(ir.find("cinder_closure_") != std::string::npos,
                    "no lifted closure function in:\n" + ir);
}

CINDER_TEST(closures_with_no_captures_allocate_nothing) {
    if (!cinder::codegen::is_available()) {
        return;
    }
    ...
    const std::string ir = compile_ir(in_main("let f = |x: int| -> int { return x; };\n"
                                              "    println(f(1));"));
    CINDER_CHECK_MSG(ir.find("cinder_alloc") == std::string::npos,
                    "a capture-free closure should not allocate:\n" + ir);
}

CINDER_TEST(closures_with_captures_allocate_and_free_an_environment) {
    if (!cinder::codegen::is_available()) {
        return;
    }
