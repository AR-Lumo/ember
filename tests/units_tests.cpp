...
using soliton::typeck::Dimension;

SourceFile unit_source(std::string contents) {
    return SourceFile{"units.sn", std::move(contents)};
}
...
SOLITON_TEST(a_dimension_is_canonical_however_it_was_written) {
    using soliton::typeck::canonical_dimension;
    ...
    SOLITON_CHECK_EQ(canonical.size(), std::size_t{2});
    SOLITON_CHECK_EQ(canonical[0].first, std::string{"meters"});
    SOLITON_CHECK_EQ(canonical[1].first, std::string{"seconds"});

    // Repeats fold together.
    SOLITON_CHECK_EQ(canonical_dimension({{"meters", 1}, {"meters", 1}}).at(0).second, 2);

    // And anything that cancels disappears entirely, rather than
    // lingering as a unit raised to the power nought.
    SOLITON_CHECK(canonical_dimension({{"meters", 1}, {"meters", -1}}).empty());
}

SOLITON_TEST(combining_dimensions_adds_and_subtracts_exponents) {
    using soliton::typeck::combine_dimensions;
    ...
    SOLITON_CHECK_EQ(speed.size(), std::size_t{2});
    SOLITON_CHECK_EQ(speed[1].second, -1);

    // A speed times a time is a distance again.
    const Dimension back = combine_dimensions(speed, seconds, 1);
    SOLITON_CHECK_EQ(back.size(), std::size_t{1});
    SOLITON_CHECK_EQ(back[0].first, std::string{"meters"});
    SOLITON_CHECK_EQ(back[0].second, 1);
}

SOLITON_TEST(a_dimension_prints_as_it_would_be_written) {
    using soliton::typeck::dimension_string;

    SOLITON_CHECK_EQ(dimension_string({{"meters", 1}, {"seconds", -1}}),
                   std::string{"meters/seconds"});
    SOLITON_CHECK_EQ(dimension_string({{"meters", 1}, {"seconds", -2}}),
                   std::string{"meters/seconds^2"});
    SOLITON_CHECK_EQ(dimension_string({{"meters", 2}}), std::string{"meters^2"});
    ...
    // back in.
    SOLITON_CHECK_EQ(dimension_string({{"seconds", -1}}), std::string{"1/seconds"});
}
...
SOLITON_TEST(matching_units_add) {
    accepts(with_units("    let a: float<meters> = 1.0<meters> + 2.0<meters>;"));
}

SOLITON_TEST(mismatched_units_do_not_add) {
...
SOLITON_TEST(multiplying_cancels_what_division_introduced) {
    accepts(with_units(
        "    let d: float<meters> = 10.0<meters>;\n"
        "    let t: float<seconds> = 2.0<seconds>;\n"
...
SOLITON_TEST(dividing_a_unit_by_itself_leaves_a_plain_number) {
    accepts(with_units(
        "    let d: float<meters> = 10.0<meters>;\n"
        "    let ratio: float = d / d;"));
}

SOLITON_TEST(scaling_by_a_plain_number_keeps_the_unit) {
    accepts(with_units(
        "    let d: float<meters> = 10.0<meters>;\n"
        "    let twice: float<meters> = d * 2.0;"));
}

SOLITON_TEST(dividing_twice_gives_a_squared_unit) {
    accepts(with_units(
        "    let d: float<meters> = 10.0<meters>;\n"
        "    let t: float<seconds> = 2.0<seconds>;\n"
...
SOLITON_TEST(a_repeated_unit_and_a_power_are_the_same_type) {
...
SOLITON_TEST(the_wrong_derived_unit_is_caught) {
...
SOLITON_TEST(an_undeclared_unit_is_an_error) {
    SOLITON_CHECK_EQ(rejects("pub fn main() {\n    let d: float<furlongs> = 1.0;\n}\n"),
                   std::string{"unknown unit `furlongs`"});
}

SOLITON_TEST(a_unit_cannot_be_declared_twice) {
    SOLITON_CHECK_EQ(rejects("unit meters;\nunit meters;\npub fn main() {}\n"),
                   std::string{"duplicate definition of unit `meters`"});
}

SOLITON_TEST(comparison_needs_matching_units) {
    accepts(with_units("    let yes = 1.0<meters> < 2.0<meters>;"));
    SOLITON_CHECK(!rejects(with_units("    let no = 1.0<meters> < 2.0<seconds>;")).empty());
}

SOLITON_TEST(int_carries_units_too) {
    accepts(with_units("    let steps: int<meters> = 7<meters>;"));
    SOLITON_CHECK(!rejects(with_units("    let bad: int<meters> = 7;")).empty());
}

SOLITON_TEST(units_travel_through_signatures) {
    accepts(
        "unit meters;\n"
        "unit seconds;\n"
...
SOLITON_TEST(a_united_argument_is_not_a_plain_one) {
    SOLITON_CHECK(!rejects("unit meters;\n"
...
SOLITON_TEST(a_unit_binds_only_when_it_touches_the_number) {
...
SOLITON_TEST(a_comparison_without_spaces_still_parses) {
...
SOLITON_TEST(a_literal_unit_parses_onto_the_literal) {
...
SOLITON_TEST(units_leave_no_trace_in_the_generated_code) {
