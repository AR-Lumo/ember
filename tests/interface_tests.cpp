...
// the module's source — the last thing standing between Soliton and
// shipping a compiled library. An interface plus an object file is a
// library; `soliton interface` writes the first and `soliton build --lib`
// writes the second.
//
...
/// will not parse.
soliton::ast::InterfaceResult interface_of(const std::string& contents) {
    const SourceFile source{"textkit.sn", contents};
    ...
    return soliton::ast::write_interface(*parsed.program, source);
}

/// The interface of a module that is meant to have one.
std::string written(const std::string& contents) {
    const soliton::ast::InterfaceResult result = interface_of(contents);
    if (!result.ok()) {
        const SourceFile source{"textkit.sn", contents};
        ::soliton::test::fail(__FILE__, __LINE__,
...
bool checks(const std::string& text) {
    const SourceFile source{"textkit.sni", text};
    const soliton::parser::ParseResult parsed = soliton::parser::parse_source(source);
    if (!parsed.ok()) {
        return false;
    }
    return soliton::typeck::check(*parsed.program, source).ok();
}
...
SOLITON_TEST(interface_keeps_public_signatures_and_drops_their_bodies) {
    const std::string text = written(
        ...
        "    return out;\n"
        "}\n");
    SOLITON_CHECK_MSG(has(text, "pub fn shout(text: string) -> String;"), text);
    SOLITON_CHECK_MSG(!has(text, "push_str"), "the body should not be here:\n" + text);
}

SOLITON_TEST(interface_leaves_out_what_is_not_public) {
    const std::string text = written("fn hidden() -> int { return 1; }\n"
                                     "pub fn shown() -> int { return 2; }\n");
    SOLITON_CHECK_MSG(has(text, "shown"), text);
    SOLITON_CHECK_MSG(!has(text, "hidden"), "a private function leaked:\n" + text);
}

SOLITON_TEST(interface_keeps_a_generic_body) {
    ...
    const std::string text = written(
        ...
        "    return out;\n"
        "}\n");
    SOLITON_CHECK_MSG(has(text, "pub fn twice<T>(value: T) -> Vec<T> {"), text);
    SOLITON_CHECK_MSG(has(text, "push(out, value);"),
                    "a generic without its body cannot be instantiated:\n" + text);
}

SOLITON_TEST(interface_keeps_a_struct_whole) {
    ...
    SOLITON_CHECK_MSG(has(text, "pub x: int"), text);
}

SOLITON_TEST(interface_keeps_a_constant_and_its_value) {
    // A caller may fold it, so the value is part of the interface.
    const std::string text = written("pub const LIMIT: int = 42;\n");
    SOLITON_CHECK_MSG(has(text, "42"), text);
}

SOLITON_TEST(interface_rebuilds_an_impl_block_around_its_public_methods) {
    const std::string text = written(
        ...
        "}\n");
    SOLITON_CHECK_MSG(has(text, "impl Point {"), text);
    SOLITON_CHECK_MSG(has(text, "pub fn get(self) -> int;"), text);
    SOLITON_CHECK_MSG(!has(text, "secret"), "a private method leaked:\n" + text);
}

SOLITON_TEST(interface_reports_a_module_with_nothing_public) {
    const soliton::ast::InterfaceResult result =
...
SOLITON_TEST(interface_refuses_a_public_signature_naming_a_private_type) {
    ...
    // declaration.
    const soliton::ast::InterfaceResult result = interface_of(
        "struct Secret { pub x: int, }\n"
        "pub fn make() -> Secret { return Secret { x: 1 }; }\n");
    SOLITON_CHECK(!result.ok());
    SOLITON_CHECK_EQ(result.diagnostics.at(0).message,
                   std::string{"`make` cannot be part of an interface"});
    SOLITON_CHECK_EQ(result.diagnostics.at(0).label, std::string{"`Secret` is not `pub`"});
}

SOLITON_TEST(interface_looks_inside_compound_types) {
    const soliton::ast::InterfaceResult result = interface_of(
        "struct Secret { pub x: int, }\n"
        "pub fn take(items: &Vec<Secret>) -> int { return len(items); }\n");
    SOLITON_CHECK_MSG(!result.ok(), "a private type inside a `&Vec<...>` should still count");
}

SOLITON_TEST(interface_accepts_the_builtin_types) {
    // `Vec` and `String` parse as ordinary named types, so they have to
    // be recognised or every signature using one looks like a leak.
    SOLITON_CHECK(interface_of("pub fn f(a: &Vec<int>, b: String) -> String { return b; }\n").ok());
}

SOLITON_TEST(interface_accepts_a_generic_parameter_as_a_type) {
    SOLITON_CHECK(interface_of("pub fn id<T>(value: T) -> T { return value; }\n").ok());
}
...
SOLITON_TEST(interface_output_is_itself_a_valid_module) {
    // The whole approach rests on this: the interface is the author's
    // own Soliton, cut at spans, so it parses because it already did.
    const std::string text = written(
        ...
        "    pub fn get(self) -> int { return self.x; }\n"
        "}\n");
    SOLITON_CHECK_MSG(checks(text), "the interface does not check:\n" + text);
}

SOLITON_TEST(interface_declarations_need_no_return_on_every_path) {
    // A signature with no body has no paths to check, and demanding a
    // return from one would make every interface an error.
    SOLITON_CHECK(checks("pub fn f() -> int;\n"));
}
...
public:
    Workspace() {
        ...
        root_ = fs::temp_directory_path() /
                ("soliton-interface-" + std::to_string(run) + "-" + std::to_string(++counter));
    ...
    std::string soliton(const std::string& arguments) const {
        return run_process("cd " + quoted(root_) + " && " + quoted(fs::path{SOLITON_BINARY}) +
                           " " + arguments)
            .output;
    }
...
SOLITON_TEST(interface_lets_a_program_build_without_the_librarys_source) {
    ...
    workspace.write("lib/textkit.sn", kLibrary);
    workspace.write("app/main.sn",
                    "import textkit;\n"
                    "pub fn main() {\n"
                    "    println(textkit::shout(\"soliton\"));\n"
                    "    println(len(textkit::twice(7)));\n"
                    "}\n");

    workspace.soliton("interface lib/textkit.sn -o dist/textkit.sni");
    workspace.soliton("build --lib lib/textkit.sn -o dist/textkit.o");
    SOLITON_CHECK_MSG(fs::exists(workspace.path("dist/textkit.sni")), "no interface was written");
    SOLITON_CHECK_MSG(fs::exists(workspace.path("dist/textkit.o")), "no object was written");

    std::error_code ignored;
    fs::remove_all(workspace.path("lib"), ignored);

    const std::string ran =
        workspace.soliton("run app/main.sn -L dist --link dist/textkit.o");
    SOLITON_CHECK_MSG(ran.find("soliton!") != std::string::npos, ran);
    // The generic was monomorphized in the *consumer's* object, from the
    // body the interface carried.
    SOLITON_CHECK_MSG(ran.find("2") != std::string::npos, ran);
}

SOLITON_TEST(interface_source_is_preferred_when_both_are_there) {
    if (!soliton::codegen::is_available()) {
        return;
    }
    ...
    const Workspace workspace;
    workspace.write("greeter.sn", "pub fn value() -> int { return 1; }\n");
    workspace.write("greeter.sni", "pub fn value() -> int;\n");
    workspace.write("main.sn",
                    "import greeter;\npub fn main() { println(greeter::value()); }\n");

    const std::string ran = workspace.soliton("run main.sn");
    SOLITON_CHECK_MSG(ran.find("1") != std::string::npos,
                    "the source should have been used, and needs no `--link`:\n" + ran);
}

SOLITON_TEST(interface_build_as_a_library_needs_no_main) {
    if (!soliton::codegen::is_available()) {
        return;
    }
    const Workspace workspace;
    workspace.write("textkit.sn", kLibrary);

    const std::string without = workspace.soliton("build textkit.sn -o out.exe");
    SOLITON_CHECK_MSG(without.find("no `main` function found") != std::string::npos, without);

    const std::string with = workspace.soliton("build --lib textkit.sn -o textkit.o");
    SOLITON_CHECK_MSG(with.find("error") == std::string::npos, with);
    SOLITON_CHECK_MSG(fs::exists(workspace.path("textkit.o")), "no object was written");
}

SOLITON_TEST(interface_library_symbols_carry_the_module_name) {
    if (!soliton::codegen::is_available()) {
        return;
    }
    ...
    const Workspace workspace;
    workspace.write("textkit.sn", kLibrary);
    workspace.soliton("build --lib textkit.sn -o textkit.o");
    ...
    SOLITON_CHECK_MSG(symbols.output.find("textkit__shout") != std::string::npos,
                    symbols.output);
}
