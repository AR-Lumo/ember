# Cinder Language — Design Spec & Build Plan

> Working name: **Cinder** (file extension `.ci`). Rename freely — it's a
...
├── libs/
│   ├── lexer/                 # cinder::lexer
│   ├── parser/                # cinder::parser — depends on lexer, produces AST
│   ├── ast/                   # cinder::ast — shared AST node types
│   ├── typeck/                # cinder::typeck — type checker, depends on ast
│   ├── codegen/                # cinder::codegen — AST/typed-AST -> LLVM IR, via LLVM's C++ API
│   └── std/                   # minimal runtime support (linked into binaries)
├── src/
│   └── main.cpp                # `cinder` binary: build/run/check subcommands
├── tests/
│   ├── golden/                 # .ci source files + expected output
│   └── snapshot_tests.cpp      # CTest-driven
└── examples/
    └── *.ci                    # sample programs, one per language feature
```
...
different problem from Cinder's own (intentionally manual) memory model.
...
Implemented as a small runtime (`cinder_std`) linked into every binary,
...
```
cinder build file.ci -o output      # compile to native executable
cinder run file.ci                  # compile + run in one step
cinder check file.ci                # type-check only, no codegen
cinder fetch                        # resolve dependencies             (v2)
cinder publish                      # add this version to the index     (v2)
cinder interface file.ci            # write the module's interface      (v2)
...
- CI-style test harness reading `.ci` files from `tests/golden/`.
...
- `cinder build` produces a real native executable (via LLVM's target
...
- `cinder run` convenience command.
...
- A module search path, so a program can import code that does not sit
  beside it.
- Package manager: `cinder.toml`, path, git and registry dependencies,
  semver requirements, and an `cinder.lock` pinning the version and
  commit each dependency resolved to. A registry is a directory of index
  files, hosted as a directory or a git repository. `cinder publish` adds
  ...
  registry. Interface files (`.cii`) let a program be built against a
...
- Rename "Cinder" / `.ci` throughout via a simple find-and-replace if
  a different name is wanted — it appears in: library/namespace names
  (`cinder::*`), the CLI binary name, the file extension check in the
  CLI, and this doc.
- Since the compiler is written in C++, prefer `std::unique_ptr` /
  ...
  memory-safe by convention even though Cinder-the-language itself has
...
These three are what would make Cinder distinctive rather than
...
would break every Cinder program ever written - and every program anyone
...
7. **`mut` is accepted and inert.** Nothing produces it, because Cinder
