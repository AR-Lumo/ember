// cinder::parser - token stream -> AST.
//
...
#ifndef CINDER_PARSER_PARSER_HPP
#define CINDER_PARSER_PARSER_HPP
...
struct Module {
    ...
    /// True when this was loaded from a `.cii` rather than a `.ci`: a
...
///
/// A module named `geometry` is looked for first as `geometry.ci` beside
...
/// Then each directory of `search` is tried two ways - `geometry.ci` for
/// a module that is one file, and `geometry/geometry.ci` for one that
...
#endif  // CINDER_PARSER_PARSER_HPP
