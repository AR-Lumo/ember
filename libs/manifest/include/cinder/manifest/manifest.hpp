// cinder::manifest - `cinder.toml`, dependency resolution, and the
// lockfile that makes a build repeatable.
//
// A package is a directory with an `cinder.toml` in it and its modules in
...
//
// `cinder.lock` records the version and the exact commit each dependency
...
#ifndef CINDER_MANIFEST_MANIFEST_HPP
#define CINDER_MANIFEST_MANIFEST_HPP
...
inline constexpr std::string_view kManifestName = "cinder.toml";

/// The file recording what a build actually used.
inline constexpr std::string_view kLockName = "cinder.lock";
...
/// Parse one `cinder.toml`. `source.path()` is used as the manifest's
/// location, so it has to be the real one.
ManifestResult parse_manifest(const ast::SourceFile& source);

/// Walk up from `start` looking for an `cinder.toml`, the way every build
...
struct ResolvedPackage {
    std::string name;
    std::string version;
    /// The package root, holding its `cinder.toml`.
    std::filesystem::path root;
    SourceKind kind = SourceKind::Path;
...
/// Read `cinder.lock`. A malformed one is an error rather than something
...
#endif  // CINDER_MANIFEST_MANIFEST_HPP
