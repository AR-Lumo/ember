// soliton::manifest - `soliton.toml`, dependency resolution, and the
// lockfile that makes a build repeatable.
//
// A package is a directory with an `soliton.toml` in it and its modules in
...
//
// `soliton.lock` records the version and the exact commit each dependency
...
#ifndef SOLITON_MANIFEST_MANIFEST_HPP
#define SOLITON_MANIFEST_MANIFEST_HPP
...
inline constexpr std::string_view kManifestName = "soliton.toml";

/// The file recording what a build actually used.
inline constexpr std::string_view kLockName = "soliton.lock";
...
/// Parse one `soliton.toml`. `source.path()` is used as the manifest's
/// location, so it has to be the real one.
ManifestResult parse_manifest(const ast::SourceFile& source);

/// Walk up from `start` looking for an `soliton.toml`, the way every build
...
struct ResolvedPackage {
    std::string name;
    std::string version;
    /// The package root, holding its `soliton.toml`.
    std::filesystem::path root;
    SourceKind kind = SourceKind::Path;
...
/// Read `soliton.lock`. A malformed one is an error rather than something
...
#endif  // SOLITON_MANIFEST_MANIFEST_HPP
