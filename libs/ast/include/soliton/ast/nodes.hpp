...
#ifndef SOLITON_AST_NODES_HPP
#define SOLITON_AST_NODES_HPP
...
enum class Effect {
    ...
    /// it yet: Soliton has no `&mut` for it to be about.
    Mut,
};
...
///
/// Parameters are unconstrained. Soliton has no traits to constrain them
...
/// Soliton annotates every other binding position, and inferring them
...
/// lives in, so `import geometry;` reads `geometry.sn` from the
