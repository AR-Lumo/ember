...
#ifndef CINDER_AST_NODES_HPP
#define CINDER_AST_NODES_HPP
...
enum class Effect {
    ...
    /// it yet: Cinder has no `&mut` for it to be about.
    Mut,
};
...
///
/// Parameters are unconstrained. Cinder has no traits to constrain them
...
/// Cinder annotates every other binding position, and inferring them
...
/// lives in, so `import geometry;` reads `geometry.ci` from the
