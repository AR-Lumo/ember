/// cinder::link - turning object files into an executable, in process.
///
...
/// machine that did the build: move `cinder` anywhere else and linking
...
/// binary. Nothing outside the Cinder install is consulted.
#ifndef CINDER_LINK_LINK_HPP
#define CINDER_LINK_LINK_HPP
...
/// directory, so `cinder` works when invoked through a symlink, through
/// PATH, or from anywhere at all.
std::filesystem::path executable_path();

/// The files the linker needs, and where they were found.
struct Toolchain {
    /// The Cinder runtime archive - `println`, allocation, panics.
...
///
/// An installed layout wins: `<prefix>/lib/cinder` beside
/// `<prefix>/bin/cinder`. Failing that, the build tree's own paths, so
...
/// Link `objects` plus the Cinder runtime into an executable.
