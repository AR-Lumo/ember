// cinder::std - the Cinder runtime, linked into every compiled binary.
//
...
//
// An Cinder `string` is a fat pointer - bytes plus length - rather than a
...
#ifndef CINDER_STD_STD_HPP
#define CINDER_STD_STD_HPP
...
const char* cinder_runtime_version(void);

// --- print: no trailing newline -------------------------------------

void cinder_print_int(int64_t value);
void cinder_print_float(double value);
void cinder_print_bool(int8_t value);
...
/// `cinder_grow` this takes exactly what was asked for rather than
