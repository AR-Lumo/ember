// soliton::std - the Soliton runtime, linked into every compiled binary.
//
...
//
// An Soliton `string` is a fat pointer - bytes plus length - rather than a
...
#ifndef SOLITON_STD_STD_HPP
#define SOLITON_STD_STD_HPP
...
const char* soliton_runtime_version(void);

// --- print: no trailing newline -------------------------------------

void soliton_print_int(int64_t value);
void soliton_print_float(double value);
void soliton_print_bool(int8_t value);
...
/// `soliton_grow` this takes exactly what was asked for rather than
