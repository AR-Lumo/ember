# Soliton Language — Design Spec & Build Plan

> Working name: **Soliton** (file extension `.sn`). Rename freely — it's a
...
├── libs/
│   ├── lexer/                 # soliton::lexer
│   ├── parser/                # soliton::parser — depends on lexer, produces AST
│   ├── ast/                   # soliton::ast — shared AST node types
│   ├── typeck/                # soliton::typeck — type checker, depends on ast
│   ├── codegen/                # soliton::codegen — AST/typed-AST -> LLVM IR, via LLVM's C++ API
│   └── std/                   # minimal runtime support (linked into binaries)
├── src/
│   └── main.cpp                # `soliton` binary: build/run/check subcommands
├── tests/
│   ├── golden/                 # .sn source files + expected output
│   └── snapshot_tests.cpp      # CTest-driven
└── examples/
    └── *.sn                    # sample programs, one per language feature
```
...
different problem from Soliton's own (intentionally manual) memory model.
...
Implemented as a small runtime (`soliton_std`) linked into every binary,
...
```
soliton build file.sn -o output      # compile to native executable
soliton run file.sn                  # compile + run in one step
soliton check file.sn                # type-check only, no codegen
soliton fetch                        # resolve dependencies             (v2)
soliton publish                      # add this version to the index     (v2)
soliton interface file.sn            # write the module's interface      (v2)
...
- CI-style test harness reading `.sn` files from `tests/golden/`.
...
- `soliton build` produces a real native executable (via LLVM's target
...
- `soliton run` convenience command.
...
- A module search path, so a program can import code that does not sit
  beside it.
- Package manager: `soliton.toml`, path, git and registry dependencies,
  semver requirements, and an `soliton.lock` pinning the version and
  commit each dependency resolved to. A registry is a directory of index
  files, hosted as a directory or a git repository. `soliton publish` adds
  ...
  registry. Interface files (`.sni`) let a program be built against a
...
- Rename "Soliton" / `.sn` throughout via a simple find-and-replace if
  a different name is wanted — it appears in: library/namespace names
  (`soliton::*`), the CLI binary name, the file extension check in the
  CLI, and this doc.
- Since the compiler is written in C++, prefer `std::unique_ptr` /
  ...
  memory-safe by convention even though Soliton-the-language itself has
...
These three are what would make Soliton distinctive rather than
...
would break every Soliton program ever written - and every program anyone
...
7. **`mut` is accepted and inert.** Nothing produces it, because Soliton
